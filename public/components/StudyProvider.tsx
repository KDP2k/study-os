...
export function StudyProvider({ children }: { children: React.ReactNode }) {
  ...
  const addDrawing = useCallback((seed: Partial<DrawingDocument> = {}) => {
    ...
    const drawing: DrawingDocument = {
      ...
      createdAt: now,
      updatedAt: now,
      pages: seed.pages || [{ id: pageId, pageNumber: 1, width: 1400, height: 900, background: "dot", elements: [] }]
    };
  ...
  const addDrawingPage = useCallback((drawingId: string) => {
    const pageId = uid("page");
    setState((current) => ({ ...current, drawings: current.drawings.map((drawing) => drawing.id === drawingId ? { ...drawing, updatedAt: new Date().toISOString(), pages: [...drawing.pages, { id: pageId, pageNumber: drawing.pages.length + 1, width: 1400, height: 900, background: "dot", elements: [] }] } : drawing) }));
    return pageId;
  }, []);
