...
export function Dashboard() {
  ...
  const personalToday = now ? state.personalEvents.filter((event) => new Date(event.startsAt).toDateString() === now.toDateString()).sort((a,b)=>+new Date(a.startsAt)-+new Date(b.startsAt)) : [];
  const personalUpcoming = state.personalEvents.filter((event)=>+new Date(event.startsAt)>=(now?.getTime()||0)).sort((a,b)=>+new Date(a.startsAt)-+new Date(b.startsAt)).slice(0,3);
    ...
    <div className="dashboard-page">
      ...
      <section className="dashboard-stat-grid">
        <article className="panel today-panel">
          ...
          {(todayEvents.length || personalToday.length) ? <div className="timeline-mini unified-timeline">{[...todayEvents.map((event)=>({kind:"course" as const,id:event.id,sort:event.start,time:formatTime(event.start),title:courseById[event.courseId]?.shortName||event.title,meta:`${event.type === "lab" ? "Lab" : event.room || "Lecture"} · ${formatTime(event.end)}`,href:`/app/courses/${event.courseId}`})),...personalToday.map((event)=>({kind:"personal" as const,id:event.id,sort:new Date(event.startsAt).toTimeString().slice(0,5),time:event.allDay?"ALL DAY":new Date(event.startsAt).toLocaleTimeString("en-CA",{hour:"numeric",minute:"2-digit"}),title:event.title,meta:`${event.category}${event.location?` · ${event.location}`:""}`,href:"/app/general"}))].sort((a,b)=>a.sort.localeCompare(b.sort)).map((item) => <Link key={`${item.kind}-${item.id}`} href={item.href}><time>{item.time}</time><span className={`timeline-dot ${item.kind}`}/><div><strong>{item.title}</strong><small>{item.meta}</small></div></Link>)}</div> : <div className="empty-state compact"><Icon name="calendar"/><p>No classes or personal events today.</p></div>}
        </article>
        ...
        <article className="panel deadline-panel">
          <div className="panel-heading"><div><span className="eyebrow">UPCOMING</span><h2>Deadlines</h2></div><Link href="/app/general">General <Icon name="arrow" size={14}/></Link></div>
          {(upcomingAssignments.length || personalUpcoming.length) ? <div className="deadline-list">{[...upcomingAssignments.map((item)=>({id:item.id,at:item.dueAt,title:item.title,meta:`${courseById[item.courseId]?.code} · ${item.weight}%`,kind:"COURSE"})),...personalUpcoming.map((event)=>({id:event.id,at:event.startsAt,title:event.title,meta:event.category,kind:"GENERAL"}))].sort((a,b)=>+new Date(a.at)-+new Date(b.at)).slice(0,5).map((item) => <div key={`${item.kind}-${item.id}`}><time>{new Date(item.at).toLocaleDateString("en-CA",{month:"short",day:"numeric"})}</time><span><strong>{item.title}</strong><small>{item.kind} · {item.meta}</small></span></div>)}</div> : <div className="empty-state compact"><Icon name="target"/><p>Add academic or General events and the dashboard will prioritize what’s next.</p></div>}
        </article>
      </section>
