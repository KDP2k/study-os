...
export function NoteWorkspace({ courseId }: { courseId: string }) {
  const { state, addNote, updateNote, deleteNote, addFlashcard, addDrawing } = useStudy();
    ...
    <div className="notes-workspace">
      ...
      <section className="note-editor-panel">
        {selected ? <>
          ...
          <div className="note-workspaces"><div className="note-workspace-head"><div><span className="eyebrow">WORKSPACE</span><strong>Handwritten / diagram pages</strong></div><button className="button micro" onClick={()=>{const id=addDrawing({courseId,noteId:selected.id,title:`${selected.title} — Workspace`});updateNote(selected.id,{drawingIds:[...(selected.drawingIds||[]),id]});setOpenDrawing(id)}}>+ DRAW PAGE</button></div><div className="note-workspace-grid">{(selected.drawingIds||[]).map((id)=>{const drawing=state.drawings.find((d)=>d.id===id);return drawing?<button key={id} onClick={()=>setOpenDrawing(id)}><DrawingPreview drawing={drawing}/><span>{drawing.title}</span></button>:null})}</div></div>
